...
const bookList =async function (req,res){
    let savedData =await BookModel.find().select({bookName:1,authorName:1,id:0}).count()
    res.send({msg:savedData})
}

const getBooksInYear =async function(req,res){
    let newyear=req.body.year
    let savedData =await BookModel.find({year:newyear})
     res.send({msg:savedData})
}
...
const getXINRBooks = async function(req,res){
 let allBooks =await BookModel.find({"prices.indianPrice":{$in:["25INR","50INr","40INR"]}})
res.send({msg:allBooks})
}
